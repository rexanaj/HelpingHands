...
const shaderModifier = (shader) => {
    ...
    shader.uniforms.smallSpeed = uniforms.smallSpeed;
    shader.uniforms.smallElevation = uniforms.smallElevation;
    ...
    shader.vertexShader = shader.vertexShader.replace(
        ...
        `
            ...
            uniform float smallSpeed;
            uniform float smallElevation;
    ...
    shader.vertexShader = shader.vertexShader.replace(
        ...
        `
            ...
            {
                elev -= abs(cnoise(vec3(position.xy * 3.0 * i, uTime * smallSpeed)) * smallElevation / i);
            }
            transformed.z = elev;
        `,
    );
...
export function Background(props) {
    const [lightPos, setLightPos] = useState([0, 0, 2]);

    useEffect(() => {
        document.addEventListener("mousemove", (e) => {
            const center_horizontal = window.innerWidth / 2;
            const center_vertical = window.innerHeight / 2;
            const offsetX = e.clientX - center_horizontal;
            const offsetY = center_vertical - e.clientY;
            const ratio = 0.014;
